...
const CategoryPlaylist = ({ id, data, nbr }) => {
  ...
  const getPlaylist = async (id) => {
    if (data !== "") {
      await axios
        ...
        .catch((err) => {
          console.log(`there is an errore to get category playlist data of id ::${id}` , err);
  ...
  if (error !== "") {
    return <div className="bg-backColor mx-12 h-32 mt-4 items-center justify-center rounded-xl " >
  ...
  return (
    <Swiper
      ...
      slidesPerView={4}
    >
      <div className="text-white ml-2 mx-4  bg-navColor grid gap-2 grid-cols-3 grid-rows-3" key='amzdamdm'>
        {playlist !== "" &&
